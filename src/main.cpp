...
uint256 hashGenesisBlock("0xc9cc494fc08e91d2160858e04fad35c5e5d438caabc7128752f516e86d08042a");
...
const string strMessageMagic = "Tenfivecoin Signed Message:\n";
...
{
    // Tenfivecoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 4 * 3600; // TenfiveCoin: Update targets every four hours
static const int64 nTargetSpacing = 1.5 * 60;  // TenfiveCoin: Blocks released every 90 seconds
...
unsigned int static GetNextWorkRequired_V1(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
  ...
  // Tenfivecoin: This fixes an issue where a 51% attack can change difficulty at will.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
  ...
  if (fTestNet) {
    if (pindexLast->nHeight + 1 >= 35) { 
      // After 100 blocks after first legacy diff adjustment
      DiffMode = 2; 
    } 
  } else {         
    if (pindexLast->nHeight + 1 >= 1000) { 
      // Let early adopters have fun. The first 1,000 blocks are using the classic difficulty algo
      // After block 1000, kimoto gravity well starts calculating blocks difficulty for us.
      DiffMode = 2; 
    }
  }
        
  // GetNextWorkRequired_V1 Legacy difficulty adjustment
  // GetNextWorkRequired_V2 Kimoto gravity well difficulty adjustment
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Tenfivecoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Tenfivecoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet) {
        ...
        hashGenesisBlock = uint256("0xa7252c37eb57ce727f5f214cc5d9c71de5af90153f088d6987f33889eff6ce1c");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        block.nNonce = 4904669;
        block.nTime = 1393650000;
        
        if (fTestNet) { 
          block.nNonce = 6802512;
        ...
        assert(block.hashMerkleRoot == uint256("0x1e000570944b98880d497aa9fac40a2bbf9fbed6d677677799bed60380c925b8"));
...
unsigned char pchMessageStart[4] = { 0xfa, 0xbf, 0xb5, 0xda }; // Tenfivecoin: increase each by adding 1 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("TenfivecoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("TenfivecoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static TenfivecoinMiner(CWallet *pwallet)
{
    printf("TenfivecoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("tenfivecoin-miner");
    ...
    try { loop {
        ...
        printf("Running TenfivecoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("TenfivecoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&TenfivecoinMiner, pwallet));
}
